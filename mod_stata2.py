...
f.write('\n\n-----------\nSTATA mod started '+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d %H:%M:%S')+'\n')
f.close() 

def BF_new_afterCreate(self):
    BF_orig_afterCreate(self)
    f = open('gamelog.txt', 'a')
    f.write('\n---------Battle starts---------- '+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d %H:%M:%S')+'\n')
...
def new_onHealthChanged(self, newHealth, attackerID, attackReasonID):
    ...
    'DMG  Player '+nameWHOdamage['name'] + ' kicked '+ EnemyVihInfo['name']+' and he has ' +str(newHealth)+' hp left \n')
    f.close() 
old_onHealthChanged = Vehicle.onHealthChanged
